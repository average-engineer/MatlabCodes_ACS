...
%Plant (2.62):  
% G(s)=200/((10*s+1)*(0.005*s+1))^2 ; Gd(s)=100/(10*s+1);
%
...
Wp2 = nd2sys([1/M 2*wb/(M^0.5) wb^2],[1 2*wb*(A^0.5) A*(wb^2)]);
Wu = 1;
systemnames = 'G Wp Wu';
inputvar = '[ r(1); u(1)]';
outputvar = '[Wp; Wu; r-G]';
...
for i=1:2
  ...
  MS=norm(S,inf,1e-4);
  MT=norm(T,inf,1e-4);
  fprintf('Design%2d: %6.2f%6.2f%6.2f%6.2f%6.1f%6.2f\n',...
